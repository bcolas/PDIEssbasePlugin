...
public class JobEntryMAXL extends JobEntryBase implements Cloneable, JobEntryInterface
{
	...
	public void loadRep(Repository rep, IMetaStore metaStore, ObjectId id_jobentry, List<DatabaseMeta> databases, List<SlaveServer> slaveServers) throws KettleException
	{
		...
		{
			throw new KettleException("Unable to load job entry of type 'maxl' from the repository with id_jobentry="+id_jobentry, dbe);
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_job) throws KettleException
	{
		...
		{
			throw new KettleException("Unable to save job entry of type 'maxl' to the repository for id_job="+id_job, dbe);
		}
	}
	...
	public Result execute(Result previousResult, int nr)
	{
		...
		if (databaseMeta!=null)
		{
			...
			try
			{
				String myMAXL = null;
				helper.connect();
				helper.openMaxlSession(UUIDUtil.getUUIDAsString());
				...
				this.logBasic(helper.getMessages());
			}
			catch(Exception e)
			{
				result.setNrErrors(1);
				try { logError(helper.getMessages()); } catch (EssException esse) { }
