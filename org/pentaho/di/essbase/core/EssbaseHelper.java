...
public class EssbaseHelper implements DatabaseFactoryInterface {
    ...
    private List<List<String>> rowValues = null;
    
	final private Level essbaseAPILogLevel;
	
	// Needed by the Database dialog
	public EssbaseHelper(){
		essbaseAPILogLevel = Level.OFF;
	}
	...
	public EssbaseHelper(final DatabaseMeta databaseMeta, final LogLevel logLevel) {
		...
		case NOTHING:
			...
			break;
		case ERROR:
			essbaseAPILogLevel = Level.ERROR;
			break;
		case MINIMAL:
		case BASIC:
			essbaseAPILogLevel = Level.WARN;
			break;
		case DETAILED:
			essbaseAPILogLevel = Level.INFO;
			break;
		case DEBUG:
			essbaseAPILogLevel = Level.DEBUG;
			break;
		case ROWLEVEL:
			essbaseAPILogLevel = Level.TRACE;
			break;
		default:
			essbaseAPILogLevel = Level.OFF;
			break;
	...
	public final void connect() throws KettleException {
		...
		try {
			...
			IEssDomain dom = ess.signOn(databaseMeta.environmentSubstitute(databaseMeta.getUsername()), databaseMeta.environmentSubstitute(databaseMeta.getPassword()), false, null, s_provider);
	...
	public void openCubeView(String name, String appName, String cubeName) throws EssException {
		this.ess = IEssbase.Home.create(IEssbase.JAPI_VERSION);
		String s_provider = "http://"+databaseMeta.environmentSubstitute(databaseMeta.getHostname())+":"+databaseMeta.environmentSubstitute(databaseMeta.getDatabasePortNumberString())+"/aps/JAPI";
		IEssDomain dom = ess.signOn(databaseMeta.environmentSubstitute(databaseMeta.getUsername()), databaseMeta.environmentSubstitute(databaseMeta.getPassword()), false, null, s_provider);
	 ...
	 */
    public void createRectangularOutput() throws EssException {
        ...
        if (this.mddata != null ) {
    		...
    		if (pageAxis != null) {
    			...
    			for (int i = 0; i < pageAxis.getTupleCount(); i++) {
    				...
    				process_Rows_Columns(mddata, pov, columnAxis, rowAxis, i);
    			}
    		} else if (rowAxis != null) {
    			process_Rows_Columns(mddata, pov, columnAxis, rowAxis, 0);
    		}
        }
    }
    
    private void process_Rows_Columns (IEssMdDataSet mddata, List<String> pov, IEssMdAxis columnAxis, IEssMdAxis rowAxis, int pageNum) throws EssException {
	...
	public String[][] getRows() {
		String[][] rows = new String[this.rowValues.size()][this.rowValues.get(0).size()];
